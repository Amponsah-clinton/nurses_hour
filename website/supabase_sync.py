...
def _get(table, order='created_at.desc'):
    """
...
def fetch_inquiry_by_id_supabase(supabase_id):
    ...
    try:
        ...
        return None, str(e)
    api = '{}/rest/v1/inquiries?select=*&id=eq.{}'.format(url, supabase_id)
    req = urllib.request.Request(
        api,
        headers={
            'apikey': key,
...
def save_user_to_supabase(user):
    """Save/upsert a user directly to Supabase public.app_users."""
    program = None
    try:
...
def save_practice_answer_to_supabase(answer):
    """Save one PracticeAnswer row to Supabase (every answer in a test)."""
    q = answer.question
